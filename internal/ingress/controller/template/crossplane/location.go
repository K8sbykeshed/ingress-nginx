...
func (c *Template) buildAllowedLocation(server *ingress.Server, location *ingress.Location, locationConfig locationCfg) ngx_crossplane.Directives {
	...
	dir = append(dir,
		...
		buildDirective(proxySetHeader, "X-Original-Forwarded-For", buildForwardedFor(c.tplConfig.Cfg.ForwardedForHeader)),
		buildDirective(proxySetHeader, "X-Original-Forwarded-Host", buildForwardedFor(c.tplConfig.Cfg.ForwardedHostHeader)),
